...
While they advocate a mixture of methods to describe the forest, they don't themselves don't machine-made classifications in this science. 

A second possibility is that we could look for patterns in the trees, branches and paths. Andrew Abbott, whose work has received much more attention in the wake of [@savage_contemporary_2009], argued more than  that a decade ago  that we should adopt pattern-based approaches to working with data:
...
- And this kind of measurement data is also dwarfed by text or image-derived data. Even relatively tightly defined data forms such as URLS. Machine learning researchers spend quite a lot of time trying to build predictive models for mundane forms of data such as URLs. They can involve millions of variables, that could be interacting in countless combinations. 
...
- In machine learning broadly speaking, there are two different ways of navigating or finding these patterns. The classification trees shown above and many other so-called 'unsupervised learning' approaches generally use a prototype-based approach to prediction. They cut up the data into local patches, and classify things according to which patch they belong to. 
...
> Working infrastructures contain multiple classification systems that are both invisible, in the senses above, and ubiquitous. The invisibility of infrastructure makes visualization or description difficult. ... Our infrastructural metaphors show how baffled we often are by these systems. They are like undergarments or tunnel dwellers. (Bowker & Star, 1999, 232) [TBA - this is not quite the right quote]

### learning to classify differently

The role of classification systems becomes much more obvious when we begin to examine the powerful presence of classification systems within machine learning. To give a less esoteric example of the work of classification as it is done in machine learning, we could look at [kittydar](http://harthur.github.io/kittydar/), a piece of cat detection software written by Heather Arthur, a software developer at Mozilla Foundation.  Kittydar is typical and unusual in certain respects. It is ususual in that Arthur works as a software developer on the Mozilla Firefox browser. Her machine learning work uses the Javascript programming language associated with web development, not with machine learning or computationally intensive applications. It is a typical machine learning application, albeit one with a quite restricted application domain -- detected of forward-facing cats in images. Image classification is a typical machine learning application. Whether it is face recognition, hand-writing recognition, or medical-image processing, pattern recognition in images or 'machine vision' is a topic of long-standing interest. Like many machine learning applications, kittydar uses 'supervised learning' techniques. In supervised learning, the algorithm is 'trained' on a set of labelled examples, and then tested against unlabelled images by asking it to label them. Kittydar implements two of the most powerful supervised learning techniques -- neural networks and support vector machines -- to locate areas of images likely to contain a cat's face.
...
>>>>>>> 163172a2b5756e51e262e783b9df45ee74b89fbb
